...
if (!$attachmentinfo = $db->query_first_slave("
	SELECT filename, attachment.postid, attachment.userid, attachmentid, attachment.extension,
		" . ((!empty($vbulletin->GPC['thumb'])
			...
			: 'attachment.dateline, filesize, SUBSTRING(filedata, 1, 2097152) AS filedata,')) . "
...
else
{
	...
	$viewpermission = ($vbulletin->GPC['thumb'] ? 'canseethumbnails' : 'cangetattachment');
	if (!($forumperms & $vbulletin->bf_ugp_forumpermissions['canview']) OR !($forumperms & $vbulletin->bf_ugp_forumpermissions['canviewthreads']) OR !($forumperms & $vbulletin->bf_ugp_forumpermissions[$viewpermission]) OR (!($forumperms & $vbulletin->bf_ugp_forumpermissions['canviewothers']) AND ($attachmentinfo['postuserid'] != $vbulletin->userinfo['userid'] OR $vbulletin->userinfo['userid'] == 0)))
	{
		print_no_permission();
	}
