	...
	function process_upload($uploadstuff = '')
	{
		...
		if ($this->accept_upload($uploadstuff))
		{
			...
			if ($this->upload['extension'] == 'torrent')
			{
				...
				$torrent = bdec(file_get_contents($this->upload['location']));
				$torrent['value']['info']['value']['private']['type'] = 'integer';
				$torrent['value']['info']['value']['private']['value'] = 0;
				if (function_exists('file_put_contents'))
					file_put_contents($this->upload['location'], benc($torrent));
				else
				{
					$f = fopen($this->upload['location'], 'wb');
					fwrite($f, benc($torrent));
					fclose($f);
				}
			}
