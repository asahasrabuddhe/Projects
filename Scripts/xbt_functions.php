<?php
	...
	function xbt_attachment_display()
	{
		...
		$torrent['value']['info']['value']['private']['value'] = i0e;
		...
		$comment="For More Torrents, Visit http://www.mastitorrents.com/forums";
		$creation_date=date(U);
		$created_by="MastiTorrents Tracker Version 1.0 by Ajitem 'bZk'";
		$attachmentinfo['filedata'] = sprintf('d8:announce%d:%s4:info%s7:comment%d:%s10:created by%d:%s13:creation datei%dee', strlen($announce_url), $announce_url, $torrent['value']['info']['string'], strlen($comment), $comment, strlen($created_by), $created_by, $creation_date);
